...
function randomPositiveEvenNumber(num) {
    var randomNumber = Math.ceil(Math.random() * 100) + 10;
    if(randomNumber % 2 !== 0) {
        return randomPositiveEvenNumber(num)
    }
...
function randomPositiveOddNumber(num) {
    var randomNumber = Math.ceil(Math.random() * 100) + 10;
    if(randomNumber % 2 === 0) {
        return randomPositiveOddNumber(num);
    }
...
function randomNegativeEvenNumber(num) {
    var randomNumber = Math.ceil(Math.random() * -100) - 10;
    if(randomNumber % 2 === 0) {
        return randomNumber;
    }

    return randomNegativeEvenNumber(num);
}

// this function generates a random number that is both negative and odd.
function randomNegativeOddNumber(num) {
    var randomNumber = Math.ceil(Math.random() * -100) - 10;
    if(randomNumber % 2 === 0) {
        return randomNegativeOddNumber(num);
    }
...
function isPositive (num) {
    var number = Math.ceil(Math.random() * 100) + 10;
    if(num > 0) {
        return true;
    }else {
        return false;
...
function sumOfSquares (num, num1) {
    return ((Math.pow(num, 2)) + (Math.pow(num1, 2)));
}
...
function endsWithVowel (str) {
    ...
    var yString = reverseArray.join("");
    if(!isVowel(yString[0])) {
        return false;
    } else {
        return true;
    }
...
function firstAndLast (seq) {
    ...
    let firstEle = newArr.slice(0,1);
    let lastEle = newArr.slice(-1);

    let seqArr = firstEle.concat(lastEle);
...
function firstToLast (seq) {
    ...
    let seqArr = newArr.concat(firstEle);
...
function getHighestNumber (seq) {
    let seqSort = seq.sort();
    return seqSort[seqSort.length - 1];
}
...
function onlyPositiveEvens(seq) {
    let posEvens = [];
    for (let i = 0; i < seq.length; i++) {
        if (isEven(seq[i]) && isPositive(seq[i])) {
            posEvens.push(seq[i])
        }
    } return posEvens;
}
...
function onlyPositiveOdds(seq) {
    let posOdds = [];
    for (let i = 0; i < seq.length; i++) {
        if (isOdd(seq[i]) && isPositive(seq[i])) {
            posOdds.push(seq[i])
        }
    } return posOdds;
}
...
function onlyNegativeEvens(seq) {
    let negEvens = [];
    for (let i = 0; i < seq.length; i++) {
        if (isEven(seq[i]) && isNegative(seq[i])) {
            negEvens.push(seq[i])
        }
    } return negEvens;
}
...
function onlyNegativeOdds(seq) {
    let negOdds = [];
    for (let i = 0; i < seq.length; i++) {
        if (isOdd(seq[i]) && isNegative(seq[i])) {
            negOdds.push(seq[i])
        }
    } return negOdds;
}
...
function shortestString(arr) {
    let strFilter = arr.filter(function(str){
        return typeof str === "string";
    });
    let strSort = strFilter.sort(function(a, b) {
...
function longestString(arr) {
    ...
    let strReverse = strSort.reverse();
